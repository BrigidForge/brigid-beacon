# AGENTS.md — Beacon AI Agent Context
...
app serving two user flows via React Router:
...
- For transaction signing on iOS: `openWalletForSigning(session)` must be called
  **synchronously before any `await`** inside each click handler so iOS Safari allows
  `window.open()` as a trusted gesture. Never move this call after an await.

### vault-ui env vars

```bash
VITE_API_BASE_URL=               # Leave unset for same-origin; set if API is on a different host
...
Real `.env.staging` and `.env.production` live on the server at
`/opt/brigidforge/repo/apps/vault-ui/` and are NOT committed. The committed
reference is `apps/vault-ui/.env.example`.
...
5. `systemctl restart beacon-api.service && beacon-worker.service`
6. Health-checks API, vault domain, and beacon domain

### Hook file locations on server

```
/var/lib/forgejo/.../brigid-beacon.git/hooks/post-receive.d/brigid-deploy
/root/update-brigid.sh
```

### Dirty-repo guard

The deploy refuses if `git status --porcelain` is non-empty. Build artifacts
(`apps/vault-ui/dist/`, `apps/vault-ui/tsconfig.tsbuildinfo`) are gitignored.
If the server is ever dirty:
...
| Unit | What it runs |
|---|---|
| `beacon-api.service` | `apps/api` — Fastify, port 3001 |
| `beacon-worker.service` | `apps/worker` — indexer + notifier |
...
Configs live in `ops/nginx/`, installed to `/etc/nginx/sites-available/` + symlinked.

| Config | Domain | Webroot |
|---|---|---|
| `vault.brigidforge.com.conf` | `vault.brigidforge.com` | `/var/www/vault` — SPA + `/api/` proxy to port 3001 |
| `staging-vault.brigidforge.com.conf` | `staging-vault.brigidforge.com` | `/var/www/staging-vault` — SPA + `/api/` proxy to staging |

`beacon.brigidforge.com` nginx points to `/var/www/vault` — same build as vault.brigidforge.com.
...
```bash
npm install
npm run dev:up                        # starts API + worker
npm run dev -w @brigid/vault-ui       # vault-ui dev server (port 5173)
npm run ci                            # full preflight before any deploy
```
...
- `apps/operator-panel-old/` — retired
- `apps/public-panel-old/` — retired
- `brigid-vault-ui-old/` — retired prototype
- `apps/viewer/` — compatibility only, not actively developed

---

## Common Pitfalls

- **"Refusing deploy: repo is dirty"** — run `git checkout -- . && git clean -fd` on server
- **iOS WalletConnect tx not opening wallet** — `openWalletForSigning(session)` must be before the first `await`
- **`VITE_WALLETCONNECT_PROJECT_ID` missing** — WalletConnect button hidden/disabled
- **nginx `try_files =404`** — breaks React Router; must be `try_files $uri $uri/ /index.html`
- **Stale git stash on server** — `git stash drop` then `git checkout -- .`
