...
function SummaryField(props: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-slate-950/45 px-4 py-4">
      ...
      <p className={`mt-2 text-sm text-white ${props.mono ? 'break-all font-mono' : ''}`}>{props.value}</p>
    </div>
  );
}
...
export function VaultSummaryPanel(props: { metadata: VaultMetadata }) {
  const [rpcHealth, setRpcHealth] = useState<{
    ...
    label: string;
    detail: string;
  }>({ tone: 'yellow', label: 'Checking', detail: 'Verifying Beacon connectivity.' });
  ...
  useEffect(() => {
    ...
    function toRpcHealth(health: OperatorHealthResponse) {
      if (health.indexer.lastErrorMessage) {
        return { tone: 'yellow' as const, label: 'Degraded', detail: health.indexer.lastErrorMessage };
      }
      if (health.indexer.isStale || health.indexer.lagBlocks > HEALTHY_LAG_BLOCKS) {
        return { tone: 'yellow' as const, label: 'Delayed', detail: `Beacon is ${health.indexer.lagBlocks} blocks behind.` };
      }
      return { tone: 'green' as const, label: 'Healthy', detail: `Responding at block #${health.chainHeadBlock}.` };
    }

    async function loadHealth() {
      try {
        const health = await fetchOperatorHealth();
        if (!cancelled) setRpcHealth(toRpcHealth(health));
      } catch (error) {
        if (!cancelled) setRpcHealth({ tone: 'red', label: 'Unavailable', detail: error instanceof Error ? error.message : 'Health check failed.' });
      }
    }
  ...
  return (
    <section className="overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.14),transparent_34%),radial-gradient(circle_at_top_right,rgba(251,191,36,0.12),transparent_30%),rgba(255,255,255,0.04)] p-8 shadow-[0_25px_90px_rgba(15,23,42,0.45)] backdrop-blur">
      <div className="space-y-2">
        ...
        <h1 className="text-2xl font-semibold text-white">Vault Identity</h1>
      </div>

      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <div className="rounded-2xl border border-white/10 bg-slate-950/45 px-4 py-4">
          <p className="text-[11px] uppercase tracking-[0.2em] text-slate-400">Vault Address</p>
          <CopyableAddress value={props.metadata.address} className="mt-2 text-sm text-white" />
        </div>
        <div className="rounded-2xl border border-white/10 bg-slate-950/45 px-4 py-4">
          <p className="text-[11px] uppercase tracking-[0.2em] text-slate-400">Owner</p>
          <CopyableAddress value={props.metadata.owner} className="mt-2 text-sm text-white" />
        </div>
      ...
      <div className="mt-4 rounded-2xl border border-white/10 bg-slate-950/45 px-4 py-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] uppercase tracking-[0.2em] text-slate-400">Beacon RPC Status</p>
              <p className="mt-1 text-sm font-medium text-white">{rpcHealth.label}</p>
            </div>
          </div>
          <p className="text-sm text-slate-300">{rpcHealth.detail}</p>
        </div>
      </div>
    </section>
  );
