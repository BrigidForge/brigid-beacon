...
export function OwnerSettings(props: { vaultAddress: string; indexedOwnerAddress: string }) {
  ...
  const [destinationLabel, setDestinationLabel] = useState('Telegram alerts');
  ...
  const duplicateSubscription = subscriptions.find((subscription) => subscription.destinationId === selectedDestinationId) ?? null;
  ...
  async function refreshOwnerData(nextSessionToken = sessionToken) {
    if (!nextSessionToken) {
      setClaimStatus({ claimed: false, claimedAt: null });
      setDestinations([]);
      setSubscriptions([]);
      setDeliveries([]);
      return;
    ...
    setSelectedDestinationId((current) =>
      destinationList.destinations.some((destination) => destination.id === current)
        ? current
        : destinationList.destinations[0]?.id || '',
    );
  }
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      void refreshOwnerData(sessionToken)
        .then(() => {
          // refreshOwnerData updates destinations via state setter; inspect the latest snapshot in a follow-up render.
        })
        .catch(() => {
          // keep polling quietly while the user finishes the bot flow
        });
    }, 4000);
  ...
  useEffect(() => {
    ...
    const linkedTelegramDestination = destinations.find((destination) => destination.kind === 'telegram');
    if (!linkedTelegramDestination) return;

    setSelectedDestinationId(linkedTelegramDestination.id);
    setAwaitingTelegramConnection(false);
    setTelegramConnectLink(null);
    setMessage(`Telegram connected. "${linkedTelegramDestination.label}" is ready for subscriptions.`);
  }, [awaitingTelegramConnection, destinations]);

  async function connectWallet() {
    const provider = getEthereumProvider();
    if (!provider) {
      throw new Error('No injected wallet detected. Open Beacon in a browser wallet session to claim this vault.');
    }
  ...
  async function handleClaim() {
    ...
    try {
      ...
      const signature = (await provider.request({
        ...
        params: [nonce.message, ownerAddress],
      })) as string;
  ...
  async function handleCreateDestination() {
    ...
    try {
      if (destinationKind === 'telegram') {
        ...
        setMessage('Telegram connect opened. Tap Start in the bot, then come back here while Beacon finishes linking the chat.');
        return;
      }
  ...
  async function handleCreateSubscription() {
    ...
    try {
      if (duplicateSubscription) {
        throw new Error('This destination already has an active subscription for the current vault.');
      }
  ...
  async function handleDisableDestination(destinationId: string) {
    ...
    try {
      await disableDestination(sessionToken!, destinationId);
      await refreshOwnerData(sessionToken!);
      setMessage('Destination disabled. Any active subscriptions tied to it were disabled too.');
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
    } finally {
      setBusy(false);
  ...
  async function handleDisableSubscription(subscriptionId: string) {
    ...
    try {
      await disableSubscription(sessionToken!, subscriptionId);
      await refreshOwnerData(sessionToken!);
  ...
  async function handleSignOut() {
    ...
    try {
      if (sessionToken) {
        await revokeOwnerSession(sessionToken);
      }
    } catch {
      // best-effort revoke
    } finally {
      ...
      setDeliveries([]);
      setBusy(false);
      setMessage('Owner session cleared on this device.');
    }
  }
      ...
      <div className="grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
        <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-5">
          ...
          <p className="mt-3 text-lg font-medium text-white">
            {claimStatus?.claimed ? 'Claimed for notifications' : 'Not yet claimed'}
          </p>
          <p className="mt-2 text-sm leading-6 text-slate-300">
            Indexed owner: <span className="font-mono text-white">{shortenAddress(indexedOwnerAddress)}</span>
          </p>
          <p className="mt-2 text-sm leading-6 text-slate-300">
            Active owner context: <span className="font-mono text-white">{shortenAddress(ownerAddress)}</span>
          </p>
          <p className="mt-2 text-sm leading-6 text-slate-300">
            Session: <span className="text-white">{hasSession ? 'active' : 'not established'}</span>
          </p>
          {!walletMatchesIndexedOwner ? (
            <p className="mt-2 text-sm text-amber-100/90">
              Connected wallet does not match the indexed vault owner. Claim and alert controls stay locked until they match.
            </p>
          ) : null}
          {claimStatus?.claimedAt ? (
            <p className="mt-2 text-sm text-emerald-100/90">Claimed at {formatIso(claimStatus.claimedAt)}</p>
          ) : null}
          <button
            type="button"
            disabled={busy || claimStatus?.claimed === true || !walletMatchesIndexedOwner}
          ...
          <div className="mt-4 grid gap-3">
            <label className="space-y-2">
              ...
              <select
                ...
                onChange={(event) => {
                  const nextKind = event.target.value as 'webhook' | 'discord_webhook' | 'telegram';
                  setDestinationKind(nextKind);
                  setTelegramConnectLink(null);
                  setAwaitingTelegramConnection(false);
                  if (nextKind === 'telegram') {
                    setDestinationLabel('Telegram alerts');
                  } else if (nextKind === 'discord_webhook') {
                    setDestinationLabel('Discord alerts');
                  } else {
                    setDestinationLabel('Ops webhook');
                  }
                }}
            ...
            {destinationKind === 'telegram' ? (
              <div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/10 px-4 py-4 text-sm text-emerald-50">
                ...
                <p className="mt-2 leading-6 text-emerald-50/90">
                  Beacon will open the official bot for you. Just tap <span className="font-semibold text-white">Start</span> in Telegram and come back here.
                </p>
                {telegramConnectLink ? (
                  <div className="mt-3 rounded-2xl border border-white/10 bg-slate-950/40 px-4 py-3 text-slate-200">
                    <p>
                      Bot: <span className="font-mono text-white">@{telegramConnectLink.botUsername}</span>
                    </p>
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (typeof window !== 'undefined') {
                          window.open(telegramConnectLink.deepLinkUrl, '_blank', 'noopener,noreferrer');
                        }
                      }}
              ...
              <label className="space-y-2">
                <span className="text-sm text-slate-300">
                  {destinationKind === 'discord_webhook' ? 'Discord webhook URL' : 'Webhook URL'}
                </span>
                <input
            ...
            <button
              type="button"
              disabled={busy || !canManage || destinationLabel.trim().length === 0 || !destinationConfigValid}
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-5">
          ...
          <div className="mt-4 space-y-3">
            {destinations.length === 0 ? (
              <p className="text-sm text-slate-400">No active destinations yet.</p>
            ) : (
              destinations.map((destination) => (
                <label
                  key={destination.id}
                  className={`block rounded-2xl border px-4 py-3 ${
                    selectedDestinationId === destination.id ? 'border-emerald-300/35 bg-emerald-300/10' : 'border-white/10 bg-white/5'
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      onClick={(event) => {
                        ...
                        void handleDisableDestination(destination.id);
                      }}
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-5">
          ...
          {selectedDestination ? (
            <p className="mt-3 text-sm text-slate-300">
              Selected destination: <span className="text-white">{selectedDestination.label}</span>
            </p>
          ) : null}
          {duplicateSubscription ? (
            <p className="mt-3 rounded-2xl border border-amber-300/30 bg-amber-300/10 px-4 py-3 text-sm text-amber-50">
              This destination already has an active subscription for this vault. Disable it first if you want to replace it.
            </p>
          ) : null}
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {EVENT_OPTIONS.map((kind) => (
              <label key={kind} className="flex items-start gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                <input
                  type="checkbox"
                  checked={selectedEventKinds.includes(kind)}
          ...
          <button
            type="button"
            disabled={busy || !canManage || !selectedDestinationId || selectedEventKinds.length === 0 || Boolean(duplicateSubscription)}
      ...
      <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-5">
        ...
        <div className="mt-4 space-y-3">
          {subscriptions.length === 0 ? (
            <p className="text-sm text-slate-400">No subscriptions yet for this vault.</p>
          ) : (
              ...
              <div key={subscription.id} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    type="button"
                    disabled={busy || !canManage}
                    onClick={() => void handleDisableSubscription(subscription.id)}
                    className="rounded-xl border border-rose-300/30 px-3 py-1 text-xs text-rose-100 disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    Disable
                  </button>
      ...
      <div className="rounded-3xl border border-white/10 bg-slate-950/45 p-5">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            onClick={() => void refreshOwnerData(sessionToken)}
        ...
        <div className="mt-4 space-y-3">
          {deliveries.length === 0 ? (
            <p className="text-sm text-slate-400">No delivery attempts yet for this vault.</p>
          ) : (
                  ...
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="text-sm font-medium text-white">
                        {formatStateLabel(delivery.eventKind)} to {delivery.destination.label}
                      </p>
                      <span
                        className={`rounded-full border px-2.5 py-1 text-[11px] uppercase tracking-[0.18em] ${deliveryTone(delivery.status)}`}
                      >
                        {formatStateLabel(delivery.status)}
                      </span>
                      <span className="rounded-full border border-white/10 bg-slate-950/60 px-2.5 py-1 text-[11px] uppercase tracking-[0.18em] text-slate-300">
                        {formatStateLabel(delivery.destination.kind)}
                      </span>
                    </div>
                    <p className="mt-2 text-sm text-slate-300">
                      {delivery.status === 'sent'
                        ? `Delivered ${delivery.deliveredAt ? formatIso(delivery.deliveredAt) : 'successfully'} after ${delivery.attemptCount ?? 1} attempt${(delivery.attemptCount ?? 1) === 1 ? '' : 's'}`
                        : `Status: ${formatStateLabel(delivery.status)}`}
                    </p>
                    {delivery.lastAttemptAt ? (
                      <p className="mt-2 text-xs text-slate-400">
                        Last attempt {formatIso(delivery.lastAttemptAt)}
                        {delivery.status !== 'sent' ? ` • Beacon will retry while the source event remains undispatched.` : ''}
                      </p>
                    ) : null}
                    {delivery.errorMessage ? (
                      <p className="mt-2 text-xs text-rose-100/90">{delivery.errorMessage}</p>
                    ) : null}
                  </div>
